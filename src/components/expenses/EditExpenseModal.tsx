...
function paiseToInput(paise: string | number): string {
...
export function EditExpenseModal({
  ...
  useEffect(() => {
    ...
    setAmount(paiseToInput(expense.amountPaise));
    setPaidByUserId(expense.paidBy.id);
    setParticipantIds(expense.shares.map((s) => s.user.id));
    setError(null);
  }, [open, expense]);
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const updated = await updateExpense(groupId, expense.id, {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Edit expense" wide>
